...
func Test_strutils_WordWrapAround(t *testing.T) {
	...
	// Test when last space position is 1
...
func Test_strutils_NumbertFmt(t *testing.T) {
	...
	// Test unsupported numeric string
...
func Test_strutils_Padding(t *testing.T) {
	...
	// Test when max length exceeds byte string length
	testStr := "test"
...
func Test_strutils_HumanByteSize(t *testing.T) {
	...
	// Test invalid unit parameter range
...
func Test_strutils_HumanFileSize(t *testing.T) {
	...
	// Test missing file descriptor error
	go func() {
		defer func() { recover() }()
	...
	// Test os.Open with cross-platform invalid path
	...
	// Test unsupported object type.(complex128)
...
func Test_strutils_AnyCompare(t *testing.T) {
	...
	// Test complex comparison in map
	...
	// Test unsupported comparison type
...
func Test_strutils_FileMD5Hash(t *testing.T) {
	...
	// Check if hash is valid MD5 (32 hex characters) instead of exact value
	// to avoid Windows/Unix line ending differences
	assert.AssertEquals(t, len(retval), 32, "MD5 hash length should be 32, got %d", len(retval))

	// Verify it's a valid hex string
	for _, char := range retval {
		isValidHex := (char >= '0' && char <= '9') || (char >= 'a' && char <= 'f') || (char >= 'A' && char <= 'F')
		if !isValidHex {
			t.Errorf("Invalid hex character in MD5 hash: %c", char)
		}
	}
